...
// DashboardManager.jsx – Fixed with analytics auto‑run, refresh buttons, and all tabs working
...
import {
  ...
  BookOpen, Code, Cpu, Server, Wrench, Info, Loader, Bot
} from "lucide-react";
import {
...
function DashboardManager({ userData, onLogout }) {
  ...
  const runAnalytics = async () => {
    ...
    // Automatically run analytics when the Analytics tab is opened
  useEffect(() => {
  ...
  const renderMultiBatchSelector = () => (
    <div className="multi-batch-selector">
      <label><Info size={16} /> Select batches to compare (auto‑updates)</label>
  ...
  // Analytics tab – now includes auto‑run and refresh button
  const renderAnalytics = () => {
    const data = analyticsData;
    // Auto‑run if no data yet (or stale)
      ...
      <div className="analytics-tab">
        <div className="section-header">
          ...
          <button className="btn-primary" onClick={runAnalytics} disabled={data.isLoading}>
            <RefreshCw size={16} /> {data.isLoading ? "Analyzing..." : "Refresh Analytics"}
          </button>
        </div>
        ...
        {data.isLoading ? <div className="loading-state"><div className="spinner" /></div> : (
          <div className="analytics-container">
            <div className="analytics-card">
              <h3><Target size={18} /> Top Skills by Proficiency</h3>
              <div className="bar-chart">
                {data.skillProficiency.map(s => (
                  <div key={s.skill} className="bar-row">
                    <span>{s.skill}</span>
                    <div className="bar" style={{ width: `${s.avgScore}%` }}>{s.avgScore.toFixed(1)}%</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="analytics-card">
              <h3><AlertCircle size={18} /> Top Skill Gaps</h3>
              <table className="mini-table">
                <thead><tr><th>Skill</th><th>Demand</th><th>Supply</th><th>Gap</th></tr></thead>
                <tbody>
                  {data.skillGaps.map(g => (
                    <tr key={g.skill}><td>{g.skill}</td><td>{g.demand}</td><td>{g.supply}</td><td>{g.gap}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
            ...
            <div className="analytics-card">
              <h3><PieChart size={18} /> Hiring Funnel</h3>
              <div className="funnel-horizontal">
                <div>Matches: {data.hiringFunnel.matches}</div>
                <div>→ Locked: {data.hiringFunnel.locked}</div>
                <div>→ Selected: {data.hiringFunnel.selected}</div>
                <div>→ Mapped: {data.hiringFunnel.mapped}</div>
              </div>
            </div>

            <div className="analytics-card">
              <h3><Award size={18} /> Interviewer Performance</h3>
              <table className="mini-table">
                <thead><tr><th>Interviewer</th><th>Interviews</th><th>Selection Rate</th></tr></thead>
                <tbody>
                  {data.interviewerPerformance.slice(0, 5).map(i => (
                    <tr key={i.name}><td>{i.name}</td><td>{i.interviews}</td><td>{i.selectionRate.toFixed(1)}%</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
            ...
            <div className="analytics-card">
              <h3><Globe size={18} /> Location Demand</h3>
              <table className="mini-table">
                <thead><tr><th>Location</th><th>Jobs</th><th>Trainees</th></tr></thead>
                <tbody>
                  {data.locationDemand.map(l => (
                    <tr key={l.location}><td>{cap(l.location)}</td><td>{l.jobCount}</td><td>{l.traineeCount}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
    ...
    return (
      <div className="job-profiles">
        <div className="section-header">
          ...
          <button className="btn-secondary" onClick={fetchJobs} disabled={jobsLoading}><RefreshCw size={16} /> Refresh</button>
        </div>
    ...
    return (
      <div className="job-details-page">
        <div className="details-nav">
          <button onClick={() => { setSelectedJobForView(null); setActiveTab("job-profiles"); }} className="back-btn"><ArrowLeft size={18} /> Back to Job Profiles</button>
        </div>
        <div className="job-details-header-card">
          <div className="header-main">
            ...
            <div className="job-title-section">
              ...
              <div className="job-meta-row">
                ...
                <span className="meta-tag"><Users size={14} /> {job.openings} openings ({job.filled} filled)</span>
                <span className="meta-tag"><Calendar size={14} /> Posted: {formatDate(job.postedDate)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="job-skills-box">
            ...
            <div className="skill-tags">
              {job.techSkills?.map(s => <span key={s} className="skill-tag">{s}</span>)}
              {job.softSkills?.map(s => <span key={s} className="skill-tag soft">{s}</span>)}
            </div>
          </div>
        </div>
              ...
              <div key={cat} className="match-category">
                ...
                <div className="matched-jobs-grid">
                  {(jobMatches?.[cat] || []).map(m => (
                    <div key={m.trainee_id} className="matched-job-card" onClick={() => setSelectedTraineeForView(m)}>
                      <div className="job-match-header">
                        <h4>{m.trainee_name}</h4>
      ...
      <div className="section-header">
        ...
        <div className="filter-options">
          ...
          <div className="filter-group">
            <select value={traineeLocationFilter} onChange={e => setTraineeLocationFilter(e.target.value)} className="filter-select">
              <option value="all">All Locations</option>
              {uniqueLocations.filter(l => l !== "all").map(l => <option key={l} value={l}>{cap(l)}</option>)}
            </select>
            <select value={traineeMappingFilter} onChange={e => setTraineeMappingFilter(e.target.value)} className="filter-select">
          ...
          <tbody>
            {filteredTrainees.map(t => (
              <tr key={t.id} onClick={() => setSelectedTraineeForView(t)} className="clickable-row">
                ...
                <td>{t.batch_name || '—'}</td>
                <td>{t.isMapped ? <span className="status-badge status-approved"><CheckCircle size={12} /> Mapped</span> : <span className="status-badge status-rejected"><AlertCircle size={12} /> Unmapped</span>}</td>
                <td><Eye size={16} /></td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="open-pool">
        ...
        {openPoolLoading ? <div className="loading-state"><div className="spinner" /></div> : filtered.length === 0 ? <div className="empty-state"><Database size={40} /><h3>No open pool trainees</h3></div> : (
          <div className="table-container">
            <table className="data-table">
              <thead>
                <tr><th>Name</th><th>Location</th><th>Batch</th><th>Avg Score</th></tr>
              </thead>
              <tbody>
                {filtered.map(t => (
                  <tr key={t.id} onClick={() => setSelectedTraineeForView(t)} className="clickable-row">
                    ...
                    <td>{cap(t.location)}</td>
                    <td>{t.batch_name || '—'}</td>
                    <td><span className="score-badge">{t.averageScore}%</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {selectedJobForSearch && (
          <>
            <div className="filters">
              <select value={searchFilters.bucket} onChange={e => setSearchFilters({ ...searchFilters, bucket: e.target.value })} className="filter-select">
          ...
          <div className="modal-overlay" onClick={() => setShowLockModal(false)}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
              ...
              <div className="modal-actions">
                ...
                <button className="btn-primary" onClick={handleLockFromSearch}>Lock</button>
              </div>
            </div>
          </div>
        )}
    ...
    return (
      <div className="skill-gap-analyzer">
        ...
        <div className="dual-chart">
          <div className="analytics-card">
            <h3><Zap size={18} /> Top Skills in Demand</h3>
            <table className="mini-table">
          ...
          <div className="analytics-card">
            <h3><CheckCircle size={18} /> Skills with Surplus Supply</h3>
            <table className="mini-table">
  ...
  const renderChatbot = () => (
    <div className="chatbot-tab">
      <div className="chat-header">
        <p className="chat-title"><Bot size={20} /> ChatBot Assistant</p>
        <div className="chat-session-controls">
          ...
          <button onClick={createNewChatSession} className="btn-primary">New Chat</button>
        </div>
      ...
      <div className="chat-container" ref={chatContainerRef}>
        <div className="chat-messages">
          ...
          {chatLoading && <div className="chat-message assistant"><div className="chat-bubble typing-indicator">...</div></div>}
        </div>
      </div>
