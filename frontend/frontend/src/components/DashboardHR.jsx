...
function DashboardHR({ userData, onLogout }) {
  ...
  // HR can only lock – no status dropdown. We'll keep only view.
  // The updateLockStatus function is removed; we'll not allow status changes from locks tab.

  // Download functions with password protection
  ...
  const executeDownload = async () => {
    ...
    try {
      ...
      if (type === 'lock-report') {
        ...
        downloadFilteredSearch(); // custom CSV generation already in function
        return;
      }
      const response = await api.get(url, { responseType: 'blob' });
      const blob = new Blob([response.data], { type: 'text/csv' });
    ...
    return (
      <div className="modal-overlay" onClick={() => setShowPasswordModal(false)}>
        <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '400px' }}>
          <div className="modal-header">
            <h3><Shield size={20} /> Data Privacy</h3>
            <button className="modal-close" onClick={() => setShowPasswordModal(false)}><X /></button>
          </div>
          <div className="modal-body">
            <p>Please enter the download password to proceed.</p>
            <input
              type="password"
              className="form-control"
      ...
      <div className="interview-locks">
        ...
        <div style={{ marginTop: '1rem', display: 'flex', justifyContent: 'flex-end' }}>
          <button className="btn-secondary" onClick={() => {
            const filtered = interviewLocks;
            if (filtered.length === 0) { toast.error('No data to download'); return; }
            // CSV generation same as before (omitted for brevity but can be added)
          }}><Download size={18} /> Download Filtered</button>
        </div>
      </div>
    );
  ...
  // Talent Search Tab – with bucket display changed to Proximity
  const renderTalentSearch = () => {
    const baseFiltered = filteredSearchMatches();
    const filtered = baseFiltered.filter(m => {
      const trainee = allTrainees.find(t => t.userId === m.trainee_id);
