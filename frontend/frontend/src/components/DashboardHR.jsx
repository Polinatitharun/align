// // DashboardHR.js – FINAL VERSION with all manager-requested enhancements
...
//   // Download functions with privacy & password
...
//       let filename = '';
//       if (type === 'lock-report') {
//         url = '/interview-locks/report/';
//         if (params.status) url += `?status=${params.status}`;
//         filename = `interview_locks${params.status ? '_' + params.status : ''}.csv`;
//       } else if (type === 'report') {
//         url = `/reports/${params.reportType}/`;
//         filename = `${params.reportType}_report.csv`;
//       } else if (type === 'interview-filtered') {
//         downloadFilteredInterviewLocks();
//         return;
//       }
...
//   const downloadFilteredInterviewLocks = () => {
//     const filtered = interviewLocks; // already filtered by lockFilter
//     if (filtered.length === 0) {
//       toast.error('No data to download');
//       return;
//     }
...
//     if (!job) return;
//     // Only show jobs with openings > 0 and active
...
//             <div style={{ marginBottom: '1.5rem', padding: '1rem', background: '#f0f9ff', borderRadius: '8px', borderLeft: '4px solid #3b82f6' }}>
//               <p style={{ fontWeight: 500, marginBottom: '0.5rem' }}>Privacy Policy Agreement</p>
//               <p style={{ fontSize: '0.9rem', color: '#1e293b' }}>
//                 By downloading this file, you agree to comply with the company's data privacy policy.
//                 The information contained is confidential and intended solely for authorised personnel.
//                 Unauthorised distribution or misuse may result in disciplinary action.
//               </p>
//             </div>
...
//             <thead><tr><th>Job Title</th><th>Department</th><th>Location(s)</th><th>Openings</th><th>Filled</th><th>Remaining</th><th>Status</th><th>Actions</th></tr></thead>
//             <tbody>
//               {jobs.map((job) => {
...
//                     <td><div className={`filled-cell ${job.filled === job.openings ? 'filled-complete' : ''}`}>{job.filled}</div></td>
//                     <td><div className="remaining-cell">{remaining}</div></td>
...
//   // Interview Locks Tab – removed status dropdown
...
//   // Selected Tab – added Cancel button
...
//           <>
//             <div className="filters-panel" style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
//               <div className="filter-group" style={{ minWidth: '150px' }}>
//                 <label style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '0.25rem', display: 'block' }}>Bucket</label>
//                 <select
...
//               </div>
//               <div className="filter-group" style={{ minWidth: '150px' }}>
//                 <label style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '0.25rem', display: 'block' }}>Location</label>
//                 <input
//                   type="text"
//                   className="form-control"
...
//                 <label style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '0.25rem', display: 'block' }}>Min Total %</label>
//                 <input
//                   type="number"
//                   className="form-control"
//                   min="0"
...
//               </div>
//               <div className="filter-group" style={{ minWidth: '150px' }}>
//                 <label style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '0.25rem', display: 'block' }}>Skill Keyword</label>
//                 <input
//                   type="text"
//                   className="form-control"
...
//             <div className="table-actions" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
//               <div>
//                 <input
//                   type="checkbox"
...
//                 {!jobHasOpenings && <span style={{ color: '#ef4444', marginLeft: '1rem' }}>(No openings left)</span>}
//               </div>
...
//                           <td><span className={`bucket-tag ${match.bucket?.toLowerCase()}`}>
...
// // DashboardHR.js – FINAL VERSION with all enhancements
// import React, { useState, useEffect } from 'react';
// import { Toaster, toast } from 'sonner';
...
//         filename = `${params.reportType}_report.csv`;
//       } else if (type === 'interview-filtered') {
//         await downloadFilteredInterviewLocks();
//         return;
//       }
...
//       const zipBlob = await zip.generateAsync({ type: 'blob', password: downloadPassword });
...
//   const downloadFilteredSearch = async () => {
//     const baseFiltered = filteredSearchMatches();
//     const filtered = baseFiltered.filter(m => {
//       const trainee = allTrainees.find(t => t.userId === m.trainee_id);
//       return !(trainee && trainee.isMapped && trainee.projectId === selectedJobForSearch.id.toString());
...
//     const csvString = csvRows.join('\n');
//     const blob = new Blob([csvString], { type: 'text/csv' });
//     const filename = `job_matches_${selectedJobForSearch?.title}_filtered.csv`;
//     const zip = new JSZip();
//     zip.file(filename, blob);
//     const zipBlob = await zip.generateAsync({ type: 'blob', password: downloadPassword });
...
//   const downloadFilteredInterviewLocks = async () => {
...
//     const csvString = csvRows.join('\n');
//     const blob = new Blob([csvString], { type: 'text/csv' });
//     const filename = `interview_locks_filtered.csv`;
//     const zip = new JSZip();
//     zip.file(filename, blob);
//     const zipBlob = await zip.generateAsync({ type: 'blob', password: downloadPassword });
//     const url = window.URL.createObjectURL(zipBlob);
...
// DashboardHR.js – FINAL VERSION with all enhancements (inline CSS removed)
import React, { useState, useEffect } from 'react';
import { Toaster, toast } from 'sonner';
import JSZip from 'jszip';
import {
                      ...
                      <>
                        {traineeMatches.total_matches === 0 ? (
                          ...
                          <>
                            ...
                            {/* Additional buckets (skills_only, location_only, nearby, no_match) can be added similarly if needed */}
                          </>
                        )}
                      </>
                    ) : (
