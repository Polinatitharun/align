            ...
            <div className="modal-body">
              <form onSubmit={(e) => { e.preventDefault(); submitFeedback(); }}>
                    ...
                    <div className="skills-tags">
                      {feedbackForm.technical_skills_assessed.map((skill, i) => (
                        <span key={i} className="skill-tag">
                          {skill}
                          <button type="button" onClick={() => handleSkillRemove(i)}>×</button>
                        </span>
                      ))}
                    </div>
                  </div>
                ...
                <div className="form-group">
                  ...
                  <textarea name="strengths" value={feedbackForm.strengths} onChange={handleInputChange} rows="2" />
                </div>

                <div className="form-group">
                  <label>Weaknesses / Areas for Improvement</label>
                  <textarea name="weaknesses" value={feedbackForm.weaknesses} onChange={handleInputChange} rows="2" />
                </div>

                <div className="form-group">
                  <label>Upskilling Needed</label>
                  <textarea name="upskill_needed" value={feedbackForm.upskill_needed} onChange={handleInputChange} rows="2" />
                </div>

                <div className="form-group">
                  <label>Overall Comments</label>
                  <textarea name="overall_comments" value={feedbackForm.overall_comments} onChange={handleInputChange} rows="3" />
                </div>
                ...
                <div className="form-group">
                  ...
                  <div className="radio-group">
                    <label>
                      <input type="radio" name="recommendation" value="selected" checked={feedbackForm.recommendation === 'selected'} onChange={handleInputChange} /> Select
                    </label>
                    <label>
                      <input type="radio" name="recommendation" value="rejected" checked={feedbackForm.recommendation === 'rejected'} onChange={handleInputChange} /> Reject
                    </label>
                  </div>
                </div>
